...
export function isActiveMember(member: Member): boolean {
  ...
  const hasValidEnddate = member.enddate.some((date: string) => date && date.trim() !== '');
  if (!hasValidEnddate) {
    return true;
  }
...
export function groupMembersByPosition(members: Member[]): Record<string, Member[]> {
  const grouped: Record<string, Member[]> = {};
  
  positionsOrder.forEach(position => {
    grouped[position] = members
      .filter((member: Member) => member.position === position && isActiveMember(member))
...
export function isAlumniMember(member: Member): boolean {
...
export function getAlumniMembers(members: Member[]): Member[] {
  return members
